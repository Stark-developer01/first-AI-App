const marketData = {
  ...
  global: {
    ...
    experienceByRole: {
      "AI/ML Engineer": { "0-2 yrs": 12, "3-5 yrs": 33, "6-9 yrs": 38, "10+ yrs": 17 },
      "Full Stack Developer": { "0-2 yrs": 41, "3-5 yrs": 37, "6-9 yrs": 16, "10+ yrs": 6 },
      "Cloud Engineer": { "0-2 yrs": 14, "3-5 yrs": 44, "6-9 yrs": 29, "10+ yrs": 13 },
      "Cybersecurity Analyst": { "0-2 yrs": 8, "3-5 yrs": 24, "6-9 yrs": 33, "10+ yrs": 35 },
      DevOps: { "0-2 yrs": 19, "3-5 yrs": 46, "6-9 yrs": 26, "10+ yrs": 9 },
    },
    stack: {
      Python: 84,
      JavaScript: 80,
  ...
  india: {
    ...
    experienceByRole: {
      "AI/ML Engineer": { "0-2 yrs": 18, "3-5 yrs": 39, "6-9 yrs": 31, "10+ yrs": 12 },
      "Full Stack Developer": { "0-2 yrs": 46, "3-5 yrs": 38, "6-9 yrs": 12, "10+ yrs": 4 },
      "Cloud Engineer": { "0-2 yrs": 22, "3-5 yrs": 42, "6-9 yrs": 26, "10+ yrs": 10 },
      "Cybersecurity Analyst": { "0-2 yrs": 17, "3-5 yrs": 29, "6-9 yrs": 34, "10+ yrs": 20 },
      DevOps: { "0-2 yrs": 33, "3-5 yrs": 44, "6-9 yrs": 17, "10+ yrs": 6 },
    },
    stack: {
      Python: 77,
      JavaScript: 85,
...
const INDIA_LIVE_POLL_INTERVAL_MS = 5 * 60 * 1000;
const INDIA_LIVE_SNAPSHOT_URL = "data/live-snapshot.json";
    ...
    [
      {
        ...
        text: global.map((d) => `${d.city}<br>Demand index: ${d.demand}`),
        hoverinfo: "text",
        marker: {
          size: global.map((d) => d.demand / 3.4),
          color: "#2f6df6",
          opacity: 0.85,
      ...
      },
      {
        ...
        text: india.map((d) => `${d.city}<br>Demand index: ${d.demand}`),
        hoverinfo: "text",
        marker: {
          size: india.map((d) => d.demand / 3.4),
          color: "#15c39a",
          opacity: 0.85,
          line: { color: "#ffffff", width: 1 },
        },
...
function updateIndiaLiveTracker(snapshot, statusLabel = "Live") {
  ...
  badge.textContent =
    statusLabel === "Baseline"
      ? `Initial snapshot at ${snapshot.timestamp} IST — ${snapshot.jobsSeen ?? "?"} jobs indexed`
...
async function fetchLiveSnapshot() {
  const response = await fetch(`${INDIA_LIVE_SNAPSHOT_URL}?t=${Date.now()}`, { cache: "no-store" });
  if (!response.ok) {
...
async function refreshIndiaLiveTracker() {
  try {
    ...
    updateIndiaLiveTracker(snapshot, snapshot.status || "Live");
  } catch {
